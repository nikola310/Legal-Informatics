...
def create_query_doc(judgements_corpus,tf_idf,sims,dictionary):
    ...
    with open(filedialog.askopenfilename(filetypes=[("Text files","*.txt")]), 'r', encoding='utf-8') as query_doc_file:
        query_doc = [query_doc_word.lower() for query_doc_word in word_tokenize(query_doc_file.read()) if query_doc_word.lower() not in serbian_stop_words] 
        query_doc_bow = dictionary.doc2bow(query_doc)
        query_doc_tf_idf = tf_idf[query_doc_bow]

        results = sims[query_doc_tf_idf]
        for result in results:
            print(judgements_corpus[result[0]]['judgement_filename'] + "," + str(results[1]))
...
def create_corpus(corpus_path):
    ...
    for filename in os.listdir(corpus_path):
        if filename.startswith('presuda_one_line_') and filename.endswith('.txt'):
            file_fullpath = corpus_path + "/" + filename
            with open(file_fullpath, 'r', encoding = 'utf-8') as judgement_file:
                judgement_text = judgement_file.read()
                judgements_corpus.append({'judgement_filename' : filename, 'judgement_text' : judgement_text})
